...
class SkillAgentTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if not os.path.isdir(_project_dir):
            yield self.create_text_message(f"❌ 项目「{project_name}」不存在，请先通过技能管理工具创建该项目。\n")
            return
        ...
        if preselected_skill_folder:
            progressive_disclosure_rules = (
                ...
                + "2) 严格按说明书执行：若说明书要求执行命令或调用脚本，必须调用 run_skill_command，不得凭已有知识跳过直接输出\n"
                + "3) 执行完成后将结果直接输出给用户\n"
            )
        ...
        system_content = (
            ...
            + "如果模型支持 function call，请直接发起工具调用；若不支持，则严格按以下 JSON 协议格式响应（不得更改字段名）：\n"
            + '调用工具：{"type":"tool","name":"<工具名>","arguments":{<参数>}}\n'
            + '例如：{"type":"tool","name":"run_skill_command","arguments":{"skill_name":"xxx","command":"xxx","cwd_relative":""}}\n'
            + '例如：{"type":"tool","name":"get_skill_metadata","arguments":{"skill_name":"xxx"}}\n'
            + '结束回复：{"type":"final","content":"最终回复内容"}\n\n'
            + "技能索引（用于判断是否需要调用技能）：\n"
        ...
        def extract_dify_sse_result(stdout: str) -> tuple[bool, str | None]:
            """
            返回 (is_dify, result)：
            - (False, None)：不是 Dify SSE
            - (True, str) ：是 Dify SSE，成功提取到结果
            - (True, None)：是 Dify SSE，但未找到 workflow_finished（接口报错/中断）
            """
            is_dify = False
            for line in stdout.splitlines():
                ...
                # 只要出现 Dify 特征字段，就标记为 Dify 接口
                if "event" in event and "task_id" in event:
                    is_dify = True

                if event.get("event") == "workflow_finished":
                    outputs = event.get("data", {}).get("outputs", {}).get("answer", {})
                    if not outputs:
                        return True, ""
                    try:
                        outputs_loads = json.loads(outputs)
                        conversation_id = event.get("conversation_id", "")
                        outputs_loads["conversation_id"] = conversation_id
                        outputs = json.dumps(outputs_loads, ensure_ascii=False, indent=2)
                    except Exception:
                        pass
        ...
        try:
            for step_idx in range(max_steps):
                ...
                if tool_calls:
                    ...
                    for tc in tool_calls:
                        ...
                        if tool_name == "get_skill_metadata":
                            ...
                            yield self.create_text_message("cwd: %s \n" % result.get("cwd"))
                            yield self.create_text_message("command: %s \n" % result.get("command"))

                            if isinstance(result, dict):
                                if result.get("returncode") is not None and int(result.get("returncode") or 0) == 0:
                                    stdout = str(result.get("stdout") or "").strip()
                                    if stdout:
                                        is_dify, dify_result = extract_dify_sse_result(stdout)
                                        if is_dify:
                                            if not dify_result:
                                                yield self.create_text_message("skill内置dify接口报错，无法提取结果")
                                            final_text = dify_result or "skill内置dify接口报错，无法提取结果"  # Dify接口：接口报错时是""，成功时是提取的内容
                                        else:
                                            final_text = stdout  # 非Dify接口：原样输出
                                        skill_done = True
                                        break

                                    else:
                                        # messages.append(UserPromptMessage(content="命令执行成功，但没有任何输出。"))
                                        # continue
                                        final_text = "命令执行成功，但没有任何输出。"
                                        break

                                else:
                                    stderr = str(result.get("stderr") or "").strip()
                                    err_msg = stderr or "命令执行失败"
                                    messages.append(
                                        UserPromptMessage(content=f"命令执行失败：{err_msg}\n请根据错误信息重新决策。"))
                                    continue
                    ...
                    if skill_done:
                        yield self.create_text_message("skill_done")
                        break
                ...
                yield self.create_text_message("action: %s \n" % str(action))
                ...
                # action不是json
                if not action or action.get("type") == "final":      # action不是json 或 action.type=final
                    if action and action.get("type") == "final":     # action是json 且 action.type=final
                        final_text = str(action.get("content") or "")
                        _dbg(f"final_json content_len={len(final_text)}")
                        break   # action=final时，相信大模型输出（但是有可能是幻觉，没调用工具）

                    else:      # action不是json，不是JSON协议格式响应
                        _dbg(f"⚠️ action并非JSON格式响应， retry={direct_text_retries} res_text: {res_text}")
                        if direct_text_retries < max_direct_text_retries:
                            ...
                            messages.append(UserPromptMessage(
                                content="你刚才直接输出了文本，请改用工具调用或 JSON 协议格式响应：\n"
                                        '{"type":"tool","name":"<工具名>","arguments":{...}}\n'
                                        '或 {"type":"final","content":"..."}'
                            ))
                            continue
                        final_text = f"⚠️ action多次并非JSON格式响应，最后一次输出：\n{res_text}"
                ...
                # action是json
                if not (action.get("type") == "tool" and action.get("name")): # action里面的type不是tool 或者 action没有name，格式不对，这两个缺一不可
                # if not action.get("name"):   # 其实不关注type，只用到了name
                    _dbg(f"⚠️ 无法识别的action 格式={action!s}")
                    if direct_text_retries < max_direct_text_retries:
                        direct_text_retries += 1
                        messages.append(AssistantPromptMessage(content=res_text))
                        messages.append(UserPromptMessage(
                            content=f"你输出的 JSON 格式不符合要求，请严格按照协议格式重新输出：\n"
                                    f'{{"type":"tool","name":"<工具名>","arguments":{{...}}}}\n'
                                    f'或 {{"type":"final","content":"..."}}'
                        ))
                        continue
                    final_text = f"⚠️ action多次JSON输出格式错误，最后一次输出：\n{res_text}"
                    _dbg(f"final_text content_len={len(final_text)}")
                    break
